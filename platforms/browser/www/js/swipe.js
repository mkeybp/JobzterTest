...
$(document).one("pagecreate", ".opret", function () {
    ...
    $(document).on("swipeleft", ".ui-page", function (event) {
        ...
        if (next && !$(this).hasClass('slide')) { //Prevents double slide from happening and Checks if form is valid ______________________insert swipe here
            navnext(next);
        }
        //HUSK SWIPE __________________________________________________________ && $(this).find('form')[0].checkValidity()
    ...
    $(document).on("click", ".next", function () {
        ...
        if (next && $(".ui-page-active").find('form')[0].checkValidity()) {
            navnext(next);
        }
    });
